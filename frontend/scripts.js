...
(function () {
    ...
    function drawTrafficLights(c, t, s, cx, cy, jx1, jx2, jy1, jy2, rh, lw) {
        var tls_dict = sumoState.traffic && sumoState.traffic.traffic_lights ? sumoState.traffic.traffic_lights : {};

        // Parse signals to get per-lane state (simplified, using compass map)
        var approachStates = {
            N: tls_dict['N'] || 'r',
            S: tls_dict['S'] || 'r',
            E: tls_dict['E'] || 'r',
        ...
        function getLaneState(dir, lane) {
            return approachStates[dir].toLowerCase();
        }
    ...
    function drawVehicles(c, t, s) {
        if (!sumoState.traffic || !sumoState.traffic.vehicles) return;
        var now = Date.now();

        // Map cluster nodes for halo rendering
        var clusterHeads = new Set();
        var maliciousNodes = new Set();
        var nodeVotes = {};

        if (sumoState.consensus && sumoState.consensus.nodes) {
            sumoState.consensus.nodes.forEach(function (nodeData) {
                if (nodeData.role === 'cluster_head') clusterHeads.add(nodeData.id);
                if (nodeData.state === 'malicious') maliciousNodes.add(nodeData.id);
                if (nodeData.vote) nodeVotes[nodeData.id] = nodeData.vote;
            });
        }

        sumoState.traffic.vehicles.forEach(function (v) {
            ...
            // ========== ROLE & VOTE RENDERING ==========
            if (clusterHeads.has(v.id)) {
                // Golden Halo for Cluster Heads
                c.strokeStyle = 'gold';
                c.lineWidth = 1 * s;
                c.beginPath();
                c.arc(0, 0, Math.max(len, wid) * 0.8, 0, Math.PI * 2);
                c.stroke();
            }
            if (maliciousNodes.has(v.id)) {
                // Red Outline for Malicious Nodes
                c.strokeStyle = '#ff3333';
                c.lineWidth = 1 * s;
                c.beginPath();
                c.arc(0, 0, Math.max(len, wid) * 0.9, 0, Math.PI * 2);
                c.stroke();
            }
            if (nodeVotes[v.id]) {
                // Small text for vote
                c.translate(0, -wid * 1.5); // Move text above car
                c.rotate(st.sa); // Unrotate text
                c.fillStyle = '#ffffff';
                c.font = (3 * s) + 'px sans-serif';
                c.textAlign = 'center';
                c.fillText(nodeVotes[v.id], 0, 0);
            }
        ...
        // Cleanup old
        var ids = new Set(sumoState.traffic.vehicles.map(v => v.id));
    ...
    mainCanvas.addEventListener('click', function (e) {
        ...
        if (sumoState.traffic && sumoState.traffic.vehicles) {
            for (var v of sumoState.traffic.vehicles) {
                var st = vehicleStates[v.id];
    ...
    function update(state) {
        $("#sumo_status").text("Step " + state.step + ", vehicles: " + state.traffic.vehicles.length);
        $("#sumo_last_update").text("Updated " + new Date().toLocaleTimeString());

        sumoState = state; // Update Global State Early

        // Format preview to include metrics without fully overwhelming DOM
        var previewData = {
            step: state.step,
            metrics: state.consensus ? state.consensus.metrics : {},
            phase: state.consensus ? state.consensus.phase : 'idle'
        };
        $("#sumo_preview").text(JSON.stringify(previewData, null, 2));

        var avg = 0;
        if (state.traffic.vehicles.length) avg = state.traffic.vehicles.reduce((a, v) => a + Number(v.speed || 0), 0) / state.traffic.vehicles.length;
        speedHistory.push(avg);
        if (speedHistory.length > 200) speedHistory.shift();

        // Process consensus real data
        if (state.consensus) {
            updateRealMetrics(state.consensus);
            if (consensusVisualizer) {
                // Mock events to keep existing UI happy momentarily, or pass raw state
                totalMessageCount++;
            }
        }
    ...
    function updateRealMetrics(consensusData) {
        if (!consensusData || !consensusData.metrics) return;

        var metrics = consensusData.metrics;

        $('#avg_latency').text(metrics.decision_latency_ms > 0 ? metrics.decision_latency_ms.toFixed(0) + 'ms' : '--');
        $('#topology_score').text(metrics.topology_stability_score > 0 ? metrics.topology_stability_score.toFixed(1) : '--');
        $('#throughput_gain').text(metrics.throughput_gain_pct > 0 ? metrics.throughput_gain_pct.toFixed(1) + '%' : '--');

        if (consensusData.phase) {
            $("#consensus_status").text(consensusData.phase.toUpperCase());
        }
    }

    function updateStatsDisplay() {
        var nodeCount = sumoState.traffic && sumoState.traffic.vehicles ? sumoState.traffic.vehicles.length : 0;
